...
/**
 * Context key backing the `view/title` show/hide-resolved buttons. Seeded on activation (not only on
 * toggle) so a user who reloads while resolved threads are hidden still gets the "Show" button.
 */
...
export async function syncResolvedContext(workspaceState: vscode.Memento): Promise<void> {
	await vscode.commands.executeCommand(
		'setContext',
		HIDE_RESOLVED_CONTEXT,
		isResolvedHidden(workspaceState),
	);
}
...
export class ConversationsWebviewProvider implements vscode.WebviewViewProvider {
	...
	/**
	 * Show/hide resolved threads. Driven by the `view/title` buttons rather than an in-pane control,
	 * matching the Files pane. Keeping the control in the title bar also means it renders independently
	 * of webview content, so it can never disappear along with the rows it filters.
	 */
	async setHideResolved(value: boolean): Promise<void> {
		await this.workspaceState.update(HIDE_RESOLVED_KEY, value);
		await syncResolvedContext(this.workspaceState);
		await this.postState();
	}
